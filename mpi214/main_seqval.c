		...
		int64_t* const pred, int64_t* const edge_visit_count_ptr, int64_t const max_used_vertex)
{
	...
	*edge_visit_count_ptr = 0; /* Ensure it is a valid pointer */
	...
	if(rank==0)
	{
		...
		fprintf(stderr, "verify_bfs_tree nedge_traversed %" PRId64 ".\n", nedge_traversed);
...
int validate_bfs_result_seq2(const tuple_graph* const tg, const int64_t nglobalverts, const size_t nlocalverts, const int64_t root,
		int64_t* const pred, int64_t* const edge_visit_count_ptr, int64_t const max_used_vertex)
{
	assert (tg->edgememory_size >= 0 && tg->max_edgememory_size >= tg->edgememory_size && tg->max_edgememory_size <= tg->nglobaledges);
	assert (pred);
	*edge_visit_count_ptr = 0; /* Ensure it is a valid pointer */
	int ranges_ok = check_value_ranges(nglobalverts, nlocalverts, pred);
	if (root < 0 || root >= nglobalverts) {
		fprintf(stdout, "%d: Validation error: root vertex %" PRId64 " is invalid.\n", rank, root);
		ranges_ok = 0;
	}
	if (!ranges_ok) return 0; /* Fail */
	...
	fprintf(stdout, "%d: validate_bfs_result_seq1\n", rank);
	...
	int64_t* restrict pred_vtx = NULL;
	{
		...
		printf("rank,nlocalverts %d %" PRId64 " max %" PRId64 " \n",rank,nlocalverts,nlocalvertsMax);

		fprintf(stdout, "%d: validate_bfs_result_seq2\n", rank);

		if(rank==0)
		{
			fprintf(stdout, "%d: validate_bfs_result_seq2.1\n", rank);

			pred_vtx = (int64_t*)xmalloc(nglobalverts * sizeof(int64_t));

			for(i=0;i<nlocalverts ;i++)
			{
				pred_vtx[vertex_to_global_for_pred(irank,i)]=get_pred_from_pred_entry(pred[i]);
			}

			fprintf(stdout, "%d: validate_bfs_result_seq2.2\n", rank);


			int64_t* pred_tmp = (int64_t*)xmalloc(nlocalvertsMax * sizeof(int64_t));
			int64_t nlocalvertsRemote;

			fprintf(stdout, "%d: validate_bfs_result_seq2.3\n", rank);

			MPI_Barrier(MPI_COMM_WORLD);

			fprintf(stdout, "%d: validate_bfs_result_seq2.4\n", rank);
			...
			for(irank=1;irank<size;irank++)
			{
				...
				MPI_Recv(pred_tmp, nlocalvertsRemote, MPI_UINT64_T, irank, 1, MPI_COMM_WORLD,
													 MPI_STATUS_IGNORE);
					printf("rank,nlocalvertsRemote %d %" PRId64 " \n",rank,nlocalvertsRemote);
			...
			xfree(pred_tmp);
			fprintf(stdout, "%d: validate_bfs_result_seq2.5\n", rank);
		}
		else
		{
			MPI_Barrier(MPI_COMM_WORLD);
			...
			for(irank=1;irank<size;irank++)
			{
				...
				{
					MPI_Send(&nlocalverts, 1, MPI_UINT64_T, 0, 0, MPI_COMM_WORLD);
                    printf("Srank,nlocalvertsRemote %d %" PRId64 " \n",rank,nlocalverts);
					MPI_Send(pred, nlocalverts, MPI_UINT64_T, 0, 1, MPI_COMM_WORLD);
				}
			}
		}
		fprintf(stdout, "%d: validate_bfs_result_seq3\n", rank);
		/*{
			int irank;
			uint64_t i;
			for(irank=0;irank<size;irank++)
			{
				...
				//if(rank==irank)
				//	for(i=0;i<nlocalverts ;i++)
				//		fprintf(stdout, "%d %" PRId64 " %" PRId64 " %" PRId64 "\n", rank,i,get_pred_from_pred_entry(pred[i]),vertex_to_global_for_pred(rank,i));
			}
		}*/
	}
    MPI_Barrier(MPI_COMM_WORLD);
	...
	if(rank==0)
	{
		...
		if(nedge_traversed<0)
		{
			fprintf(stdout, "Validation error: code %" PRId64 ".\n", nedge_traversed);
			validation_passed=0;
		}
	}
	if(rank==0)
	...
	fprintf(stdout, "%d: validate_bfs_result_seqF\n", rank);
	MPI_Barrier(MPI_COMM_WORLD);
	MPI_Allreduce(MPI_IN_PLACE, &nedge_traversed, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
	*edge_visit_count_ptr=nedge_traversed;
	/* Collect the global validation result. */
...
int main(int argc, char** argv) {
  ...
  if ( !(argc == 2 || argc == 3)){
	  if (rank == 0)
		  fprintf(stdout, "Usage: %s input_file [number of threads]\n", argv[0]);
...
#else
	  if(threads!=1)
		  fprintf(stdout, "ERROR: %s compiled without OpenMP\n", argv[0]);
#endif
  }

  if (rank == 0)
  	  fprintf(stdout, "Graph500 version: 2.1.4 replicated csc\n");

  {
	  ...
	  if(input_file==NULL){
		  if (rank == 0)
			  fprintf(stdout, "Error : can no open %s file\n",argv[1]);
		  MPI_Barrier(MPI_COMM_WORLD);
		  MPI_Abort(MPI_COMM_WORLD, 1);
	  }
	  ...
	  fgets(cbuf,256,input_file);iRead+=sscanf(cbuf,"%d",&bUsePerfForTEPS);
	  ...
	  if (rank == 0){
		  ...
		  fprintf(stdout, "\tNumber of BFS roots: %d\n",num_bfs_roots);
		  fprintf(stdout, "\tUse pure OpenMP Implementation for single node: %d\n",bDoOneNodePureOpenMP);
		  fprintf(stdout, "\tRun performance section: %d\n",bRunPerf);
		  fprintf(stdout, "\tRun validation: %d\n",bRunVal);
		  fprintf(stdout, "\tTime for performance section in seconds: %f\n",timeForPerf);
		  fprintf(stdout, "\tUse only performance section for TEPS calculation: %d\n",bUsePerfForTEPS);
		  fprintf(stdout, "\tMax number of cycles: %d\n",numberOfCyclesForPerf);
		  fprintf(stdout, "\tNumber of MPI processes: %d\n",size);
#ifdef _OPENMP
		  fprintf(stdout, "\tMax number of threads per MPI process: %d\n",omp_get_max_threads());

#else
		  fprintf(stdout, "\tMax number of threads per MPI process: compiled without OpenMP\n");
#endif
		  //fprintf(stdout, "\tReffrence md5 on initial edge list: ");

		  //for (i = 0; i < 16; i++)
		  //  fprintf(stdout, "%2.2x", refMD5[i]);
		  //fprintf(stdout, "\n");
...
#pragma omp atomic
  ...
  while(1){
	  MPI_Barrier(MPI_COMM_WORLD);
	  if (rank == 0)fprintf(stdout, "Starting cycle %d.\n", CyclesPassed);

	  for (bfs_root_idx = 0; bfs_root_idx < num_bfs_roots; ++bfs_root_idx) {
		  int64_t root = bfs_roots[bfs_root_idx];

		  if ((rank == 0)&&(ValidationStep)) fprintf(stdout, "Running BFS %d\n", bfs_root_idx);
	    ...
	    run_bfs(root, &pred[0]);
		  ...
		  if( (!ValidationStep) || (bUsePerfForTEPS==0)){
	...
	#endif			  //int validation_passed_one = validate_bfs_result(&tg, max_used_vertex + 1, nlocalverts, root, pred, &edge_visit_count);
			  MPI_Barrier(MPI_COMM_WORLD);
  ...
  for (bfs_root_idx = 0; bfs_root_idx < num_bfs_roots; ++bfs_root_idx) {
	  bfs_times[bfs_root_idx]/=(count/num_bfs_roots);
  }
...
#ifdef SHOWCPUAFF
  {
	  ...
	  pid_t pid=getpid();
  	  for (i = 0; i < size; i++){
  		  if(i==rank){
  			fprintf(stdout, "MPI Process %d, memory usage:\n",rank);
  			sprintf(cbuf,"grep -i Vm /proc/%d/status",pid);
  			system(cbuf);
  			fprintf(stdout, "\n");
  		  }
  		  MPI_Barrier(MPI_COMM_WORLD);
